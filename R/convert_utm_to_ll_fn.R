...
#'
#' @param eastings eastings of points
#' @param northings northings of points
...
#'
#' b <- convert_utm_to_ll_fn(eastings = eastings, northings = northings, zone = zone, flip_around_dateline = flip_around_dateline)
#'
#' ggmap::qmplot(x = long, y = lat, data = b, color = 'red')
...
convert_utm_to_ll_fn <-
  function(eastings,
    ...
    ll_coords <-
      sf::st_transform(utm_coords, crs = "+proj=longlat +datum=WGS84")
    ...
    ll_coords <-  purrr::map_df(ll_geometry, extract_ll)
    ...
    # blah <- data_frame(badlon = c(-177, 120,-2,22, 22.3,-56.89)) %>%
    #   mutate(flipped = map_dbl(badlon, ~180 + ifelse(.x > 0, .x - 360, .x))) %>%
    #   mutate(unflip = map_dbl(flipped, ~ ifelse(.x > 0, .x - 180, .x + 180)))
